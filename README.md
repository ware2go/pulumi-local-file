...
First, clone this repo with the name of the desired provider in place of `localfile`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-localfile
```

Second, replace references to `localfile` with the name of your provider:
...
To use from Go, use `go get` to grab the latest version of the library
    ...
    $ go get github.com/pulumi/pulumi-localfile/sdk/go/...

## Configuration

The following configuration points are available for the `localfile` provider:

- `localfile:apiKey` (environment: `XYZ_API_KEY`) - the API key for `localfile`
- `localfile:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
